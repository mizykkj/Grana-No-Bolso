...
// --- CONTROLES CORRIGIDOS PARA INICIAR COM SETAS E PREVENIR ROLAGEM ---
function keyDownHandlerQB(e) {
    const keyLower = e.key.toLowerCase();
    const isRight = keyLower === 'arrowright' || keyLower === 'd';
    const isLeft = keyLower === 'arrowleft' || keyLower === 'a';
    const isActionKey = ['enter', ' ', 'arrowup', 'arrowdown'].includes(keyLower);
    const isMoveKey = isRight || isLeft;

    if (isMoveKey || isActionKey) {
        e.preventDefault();
    }
    
    if (!gameHasStartedQB && isMoveKey) {
        initializeGameQuebraBlocos();
    }
    
    if (gameHasStartedQB && !gameOver && !gamePaused) {
        if (isRight) rightPressedQB = true;
        else if (isLeft) leftPressedQB = true;
    }
    
    if (gamePaused && (isActionKey || isMoveKey)) {
...
function keyUpHandlerQB(e) {
    ...
    if (isRight) rightPressedQB = false;
    else if (isLeft) leftPressedQB = false;
}
// -------------------------------------------------------------------
...
function updateQB() {
    ...
    if (ballY + ballRadius > canvas.height - paddleHeight && // Bola está na altura vertical do paddle
