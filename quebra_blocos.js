...
function resetBallAndPaddleQB() { const initialBallSpeedMagnitude = 3.5; ballX = canvas.width / 2; ballY = canvas.height - paddleHeight - ballRadius - 25; paddleX = (canvas.width - paddleWidth) / 2; let speedMultiplier = 1 + (currentLevel - 1) * 0.18; let currentSpeedMagnitude = initialBallSpeedMagnitude * speedMultiplier; if (currentSpeedMagnitude > MAX_BALL_SPEED_MAGNITUDE_QB) currentSpeedMagnitude = MAX_BALL_SPEED_MAGNITUDE_QB; if (currentSpeedMagnitude < initialBallSpeedMagnitude) currentSpeedMagnitude = initialBallSpeedMagnitude; let angle = (Math.PI / 4) + (Math.random() * Math.PI / 2); ballSpeedX = currentSpeedMagnitude * Math.cos(angle) * (Math.random() > 0.5 ? 1: -1); ballSpeedY = -currentSpeedMagnitude * Math.sin(angle); if (Math.abs(ballSpeedX) < 1.5) ballSpeedX = (ballSpeedX >= 0 ? 1.5 : -1.5); if (ballSpeedY >= 0) ballSpeedY = -1.5; }
...
function keyDownHandlerQB(e) { const MOVE_KEYS = ['ArrowLeft', 'ArrowRight', 'a', 'A', 'd', 'D']; const ACTION_KEYS = ['Enter', ' ', 'ArrowUp', 'ArrowDown', ...MOVE_KEYS]; if (!gameHasStartedQB && MOVE_KEYS.some(k => k.toLowerCase() === e.key.toLowerCase())) { e.preventDefault(); initializeGameQuebraBlocos(); } if (gameHasStartedQB && !gameOver && !gamePaused) { if (e.key === 'ArrowRight' || e.key.toLowerCase() === 'd') rightPressedQB = true; else if (e.key === 'ArrowLeft' || e.key.toLowerCase() === 'a') leftPressedQB = true; if (MOVE_KEYS.some(k => k.toLowerCase() === e.key.toLowerCase())) e.preventDefault(); } if (gamePaused && (ACTION_KEYS.includes(e.key) || e.code === 'Space')) { e.preventDefault(); if(nextLevelButtonQB) nextLevelButtonQB.click(); }}
function keyUpHandlerQB(e) { if (e.key === 'Right' || e.key === 'ArrowRight' || e.key.toLowerCase() === 'd') rightPressedQB = false; else if (e.key === 'Left' || e.key === 'ArrowLeft' || e.key.toLowerCase() === 'a') leftPressedQB = false; }
...
function updateQB() {
    ...
    if (ballY + ballSpeedY < ballRadius) {
        ...
        if (ballX > paddleX && ballX < paddleX + paddleWidth) { // Bola está na horizontal do paddle?
