// quebra_blocos.js - VERSÃO COM LOGS DE DEBUG PARA DESENHO

const canvas = document.getElementById('gameCanvas');
const ctx = canvas.getContext('2d');

// ... (seletores de elementos de UI como antes) ...
...
// --- FUNÇÕES DE DESENHO COM LOGS ---
function drawBallQB() {
    if (!ctx) return;
    console.log(`DEBUG: Desenhando bola em (x:${ballX}, y:${ballY})`);
    ctx.beginPath();
...
function drawPaddleQB() {
    if (!ctx) return;
    console.log(`DEBUG: Desenhando barra em (x:${paddleX})`);
    ctx.beginPath();
...
function drawBricksQB() {
    if (!ctx) return;
    console.log(`DEBUG: Tentando desenhar ${brickRowCount}x${brickColumnCount} tijolos.`);
    let bricksDrawn = 0;
    ...
    console.log(`DEBUG: ${bricksDrawn} tijolos foram desenhados.`);
}

// --- FUNÇÕES DE SETUP E LÓGICA (completas) ---
...
function increaseBallSpeed() { let currentMagnitude = Math.sqrt(ballSpeedX*ballSpeedX + ballSpeedY*ballSpeedY); let newMagnitude = currentMagnitude + SPEED_INCREMENT; ballSpeedX = (ballSpeedX / currentMagnitude) * newMagnitude; ballSpeedY = (ballSpeedY / currentMagnitude) * newMagnitude; console.log("Velocidade aumentada para:", newMagnitude.toFixed(1)); }
