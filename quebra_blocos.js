// quebra_blocos.js - ATUALIZADO com nova lógica de velocidade e colisão
...
let score = 0, lives = INITIAL_LIVES, currentLevel = 1;
...
let blocksBrokenSinceLastSpeedIncrease = 0; // NOVO: Contador de blocos quebrados
const BLOCKS_FOR_SPEED_INCREASE = 8;        // NOVO: Aumenta a velocidade a cada 8 blocos
const SPEED_INCREMENT = 0.25;                 // Aumento sutil
...
function keyDownHandlerQB(e) { const MOVE_KEYS = ['ArrowLeft', 'ArrowRight', 'a', 'd']; const ACTION_KEYS = ['Enter', ' ', 'ArrowUp', 'ArrowDown', ...MOVE_KEYS]; if (!gameHasStartedQB && MOVE_KEYS.includes(e.key.toLowerCase())) { e.preventDefault(); initializeGameQuebraBlocos(); } if (gameHasStartedQB && !gameOver && !gamePaused) { if (e.key === 'ArrowRight' || e.key.toLowerCase() === 'd') rightPressedQB = true; else if (e.key === 'ArrowLeft' || e.key.toLowerCase() === 'a') leftPressedQB = true; if (MOVE_KEYS.includes(e.key.toLowerCase())) e.preventDefault(); } if (gamePaused && (ACTION_KEYS.includes(e.key) || e.code === 'Space')) { e.preventDefault(); if(nextLevelButtonQB) nextLevelButtonQB.click(); }}
function keyUpHandlerQB(e) { if (e.key === 'Right' || e.key.toLowerCase() === 'd') rightPressedQB = false; else if (e.key === 'Left' || e.key.toLowerCase() === 'a') leftPressedQB = false; }
...
function increaseBallSpeed() {
    ...
    console.log("Velocidade aumentada por quebrar blocos para:", newMagnitude.toFixed(1));
}
        ...
        for (let r = 0; r < brickRowCount; r++) {
            ...
            if (b.status === 1) {
                ...
                if (ballX + ballRadius > b.x && ballX - ballRadius < b.x + brickWidth && ballY + ballRadius > b.y && ballY - ballRadius < b.y + brickHeight) {
                    ...
                    // --- LÓGICA DE AUMENTO DE VELOCIDADE POR BLOCO QUEBRADO ---
                    blocksBrokenSinceLastSpeedIncrease++;
                    if (blocksBrokenSinceLastSpeedIncrease > 0 && blocksBrokenSinceLastSpeedIncrease % BLOCKS_FOR_SPEED_INCREASE === 0) {
                        increaseBallSpeed();
                    }
                    // --------------------------------------------------------
                }
...
function updateQB() {
    ...
    // --- LÓGICA DE COLISÃO COM PADDLE E CHÃO CORRIGIDA E SIMPLIFICADA ---
    if (ballY + ballSpeedY < ballRadius) {
        ballSpeedY = -ballSpeedY; // Colisão com teto
    } else if (ballY + ballSpeedY > canvas.height - ballRadius) { // Bola está na parte de baixo, perto ou depois da altura da barra
        if (ballX > paddleX && ballX < paddleX + paddleWidth) {
            // Colisão com o paddle!
            ballSpeedY = -ballSpeedY;
            // REMOVIDA: Lógica de ângulo e aumento de velocidade na rebatida. É uma rebatida simples.
        } else {
    ...
    else if (leftPressedQB && paddleX > 0) leftPressedQB -= PADDLE_SPEED_QB;
...
if (nextLevelButtonQB) {
    nextLevelButtonQB.addEventListener('click', () => {
        ...
        blocksBrokenSinceLastSpeedIncrease = 0; // Reseta contador de blocos quebrados no novo nível
    });
}
