...
function keyDownHandlerQB(e) {
    ...
    if (!gameHasStartedQB && MOVE_KEYS.includes(e.key.toLowerCase())) { e.preventDefault(); initializeGameQuebraBlocos(); }
    ...
    if (gamePaused && (ACTION_KEYS.includes(e.key) || e.code === 'Space')) { e.preventDefault(); if(nextLevelButtonQB) nextLevelButtonQB.click(); }
}

function keyUpHandlerQB(e) {
    if (e.key === 'Right' || e.key.toLowerCase() === 'd') rightPressedQB = false;
    else if (e.key === 'Left' || e.key.toLowerCase() === 'a') leftPressedQB = false;
}
...
function updateQB() {
    ...
    // Colisão com a parte de baixo (paddle ou chão) - LÓGICA CORRIGIDA
        ...
        } else if (ballY + ballRadius > canvas.height) { // Se passou da altura do paddle e chegou ao fundo
