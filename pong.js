// pong.js - VERSÃO FINAL COM IA E CORREÇÕES
...
function initializePongVariables() {
    ...
    ballSpeedX = baseBallSpeedX * (Math.random() > 0.5 ? 1 : -1);
    ballSpeedY = baseBallSpeedY * (Math.random() > 0.5 ? 1 : -1);
    resetBallPosition(); updateUIDisplays();
}
...
function resetBallPosition() { ballX = canvas.width / 2; ballY = canvas.height / 2; }
function resetBall(scorer) {
    ...
    ballSpeedX = speedMagnitude * 0.9 * directionX;
    ballSpeedY = speedMagnitude * 0.4 * (Math.random() > 0.5 ? 1 : -1);
    lastHitBy = null;
}
